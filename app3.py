...
def initialize_session_state():
    ...
    if "summarized_chat_history" not in st.session_state:
        st.session_state.summarized_chat_history = ""
...
def process_resumes(uploaded_files):
    ...
    try:
        save_dir = (
            r"C:\Users\Azeem\Documents\CARE\Intelligent-Resume-Filtering\Uploaded_Files"
        )
...
• Company culture and values""",
    ...
    with col1:
        top_k = st.slider(
            "Number of candidates to find", 1, len(st.session_state.candidates), 1
        )
    ...
    with col2:
        if st.button(
            "🔍 Find Matching Candidates", type="primary", use_container_width=True
        ):
...
def analyze_job_description(job_description, top_k):
    run_analysis_pipeline(
        job_description=job_description, top_k=top_k, mode="selection"
    )
...
def render_candidates_page():
    ...
    if not st.session_state.candidates:
        st.warning(
            "No candidates found. Please complete job description analysis first."
        )
        return
    ...
    if st.session_state.selected_candidates:
        ...
        st.success(
            f"Selected {len(st.session_state.selected_candidates)} candidates: {', '.join(map(str, st.session_state.selected_candidates))}"
        )
...
def process_chat_message(user_input):
    ...
    try:
        with st.spinner("AI is thinking..."):
            selected_candidates = filter_selected_candidates(
                st.session_state.candidates, st.session_state.selected_candidates
            )
            print(f"Selected candidates for chat: {selected_candidates}")

            response, chat_history, summarized_chat = normal_chatbot(
                st.session_state.chat_history,
                st.session_state.summarized_chat_history,
                user_input,
                st.session_state.job_description,
        ...
        # Persist updated histories
        st.session_state.summarized_chat_history = summarized_chat
        st.session_state.chat_history = chat_history

        # Update display-only history
        if "chat_display_history" not in st.session_state:
            st.session_state.chat_display_history = []
